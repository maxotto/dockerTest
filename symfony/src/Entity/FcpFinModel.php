 ...
 * FcpFinModel
 *
 * @ORM\Table(name="fcp_fin_model", indexes={@ORM\Index(name="fcp_object_id", columns={"fcp_object_id"})})
 * @ORM\Entity
 */
class FcpFinModel
{
     ...
     * @var string|null
     *
     * @ORM\Column(name="prim", type="string", length=255, nullable=true, options={"comment"="Примечание"})
     */
    private $prim = '';
     ...
     * @var int
     *
     * @ORM\Column(name="ams_included_in", type="integer", nullable=false, options={"unsigned"=true,"comment"="Признак, куда включена стоимость АМС. По умолчанию - в СМР (0)"})
     */
    private $amsIncludedIn = '0';

    /**
     * @var int
     *
     * @ORM\Column(name="smeta_source", type="integer", nullable=false, options={"unsigned"=true,"comment"="Источник сметы.  0 - первичная,2 - по итогам повторной экспертизы, 2 - переутверждение в РосПечати"})
     */
    private $smetaSource = '0';
    ...
    public function getAmsIncludedIn(): ?int
    {
        return $this->amsIncludedIn;
    }

    public function setAmsIncludedIn(int $amsIncludedIn): self
    {
        $this->amsIncludedIn = $amsIncludedIn;

        return $this;
    }

    public function getSmetaSource(): ?int
    {
        return $this->smetaSource;
    }

    public function setSmetaSource(int $smetaSource): self
    {
        $this->smetaSource = $smetaSource;
